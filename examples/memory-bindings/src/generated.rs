...
pub unsafe fn open() -> Result<Fd> {
    let mut opened_fd = MaybeUninit::uninit();
    let rc = wasi_ephemeral_memory::open(opened_fd.as_mut_ptr());
...
pub mod wasi_ephemeral_memory {
    use super::*;
    #[link(wasm_import_module = "wasi_ephemeral_memory")]
    extern "C" {
        /// Open a memory handle.
        pub fn open(opened_fd: *mut Fd) -> Errno;
    }
}
